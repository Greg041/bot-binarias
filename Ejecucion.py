...
def ejecucion(signal: str, par: str, tiempo: str, monto=None):
    if signal != "":
        if par == "EUR_USD":
            ...
            pyautogui.leftClick(x=1072, y=265)
            pyautogui.leftClick(x=1072, y=265)
            click_image("par_aud_usd.jpg", (887, 320), "left", 0.05)
        time.sleep(1)
    if signal == "comprac":
        ...
        pyautogui.keyDown(tiempo)
        time.sleep(2)
        click_image("imagen compra.jpg", (1088, 502), "left", 0.05)
        winsound.Beep(440, 1000)
    elif signal == "ventaf":
        print("venta a favor")
        print(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
