from ExtraccionDatosBroker import Extraccion
...
def run(tiempo_de_ejecucion_minutos):
    divisa = "EUR_USD"
    # hilo_5_min = Process(target=Extraccion, args=(100, "M5", divisa))
    # hilo_1_min = Process(target=Extraccion, args=(500, "M1", divisa))
    # hilo_5_min.start()
    # hilo_1_min.start()
    ...
    # tiempo_ejecucion = tiempo_de_ejecucion_minutos * 60
    # timeout = time.time() + tiempo_ejecucion
    ...
    while time.time() <= timeout:
        if time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())) == "2019-12-29 16:59:00"[17:]:
            print("hola")
        # datos_5_min = pd.read_csv("datos M5", index_col="time")
        # datos_1_min = pd.read_csv("datos M1", index_col="time")
        # resistencia_max_5_min = datos_5_min["h"].rolling(100).max()[-1]
        # soporte_max_5_min = datos_5_min["l"].rolling(100).min()[-1]
    #     starttime = time.time()
    ...
    #     last_data_row['o'] = round(last_data_row['o'], 5)
    #     last_data_row['h'] = round(max(rango_precios), 6)
    #     last_data_row['h'] = round(last_data_row["h"], 5)
    #     last_data_row['l'] = round(min(rango_precios), 6)
    #     last_data_row['l'] = round(last_data_row['l'], 5)
    #     last_data_row['c'] = round(rango_precios[-1], 6)
    #     last_data_row['c'] = round(last_data_row["c"], 5)
